...
class AgentCorePracticeAsyncLauncher:
    ...
    def _run_tracked_with_context(self, task_id: int, test_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                "practice background task failed test_id=%s error_type=%s",
                test_id,
                failure_class,
            )
            emit_practice_event(
...
def build_practice_async_launcher(
    *,
    ...
    orchestrator = PracticeGenerationOrchestrator(
        ...
        generator=RoutedQuestionGenerator(llm_orchestrator),
        verifier=RoutedQuestionVerifier(llm_orchestrator),
        pattern_context=build_pattern_context_provider(enabled=config.pattern_context_enabled),
    )
